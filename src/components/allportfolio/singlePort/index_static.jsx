...
export default function StaticSinglePort(props) {
  const {program} = props
  const id = Number(program.num)
  const [commentNum,setcommentNum] = useState(program.comments.length)
  const [likeNum,likeNumNum] = useState(program.like)
  const [seenNum,seenNumNum] = useState(program.seen)
  useEffect(()=>{
    axios({
        method:"get",
        // url:`http://localhost:8000/allportfolio/${id}`,
        url:`http://54.151.228.224:8000/allportfolio/${id}`,
    }).then((res)=>{
        let commentslen = res.data.comments.length
        setcommentNum(commentslen )
        let likes = res.data.like
        likeNumNum(likes)
        let seens = res.data.seen
        seenNumNum(seens)
    }).catch((err)=>{
        console.log(err)
    })
  ...
  return (
    <article className="allsingleport__content">
        <NavLink to={`/detailportfolio/${program.num}`}>
        <div>
        ...
        <section className="singleport__section">
            ...
            <div className="singleport__righticonbox">
                <div className="righticons__empty"></div>
                <div className="singleport__righticons">
                    <div className="singleport__righticon">
                        <svg t="1682254927791" className="singleport__icon__right" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2699">
                    ...
                    <div className="singleport__righticon">
                        ...
                        <span className="singleport__righticonnum">{seenNum}</span>
                    </div>
